...
Proof.
  ...
  - destruct Ht as [_ Ht]; cbn in Ht.
    unfold annotated_transition in Ht; destruct (vtransition _ _ _); inversion Ht.
...
Proof.
  ...
  - destruct s; cbn.
    replace state_annotation with (@nil validator)
      by (symmetry; apply Hs).
    destruct threshold; cbn; apply Rge_le; assumption.
...
Lemma message_equivocators_can_emit (s : vstate Limited) im
  ...
  : can_emit
      ...
      im.
Proof.
  unfold msg_dep_message_equivocators, coeqv_message_equivocators; rewrite decide_False by assumption.
        ...
        can_emit (pre_loaded_with_all_messages_vlsm (IM dm_i)) dm).
  {
        ...
        can_emit (pre_loaded_with_all_messages_vlsm (IM i)) dm).
    {
      ...
      exists i; split; [|assumption]; subst.
      apply elem_of_app; right.
      apply elem_of_map_option.
      exists dm; split; [|assumption].
      apply elem_of_list_filter; split; [assumption|].
    ...
    revert HLemit; eapply VLSM_full_projection_can_emit.
  ...
  eapply VLSM_full_projection_can_emit.
  {
    ...
    Unshelve.
    exact (set_union_subseteq_right (state_annotation s) equivocating_senders).
  }
  assert
...
Proof.
  ...
  - setoid_rewrite annotate_trace_from_app; cbn.
    ...
    + apply finite_valid_trace_from_app_iff.
      ...
      * destruct l as (i, li); cbn; rewrite annotate_trace_from_last_original_state; cbn.
        replace (finite_trace_last _ _) with s
          by (apply valid_trace_get_last in Htr1; congruence).
        apply Ht.
      ...
      * destruct l as (i, li); cbn; unfold annotated_transition; cbn.
        ...
        replace (finite_trace_last _ _) with s
          by (apply valid_trace_get_last in Htr1; congruence).
        apply proj2 in Ht; cbn in Ht.
        destruct (vtransition _ _ _) as (si', om'); inversion Ht; reflexivity.
    + eapply fixed_transition_preserves_annotation_equivocators
      ; [eassumption|eassumption|apply IHHtr1].
Qed.
...
Proof.
  ...
  eapply traces_exhibiting_limited_equivocation_are_valid ; [apply Hsender_safety|].
