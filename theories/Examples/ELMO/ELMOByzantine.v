...
| existT _ Receive, Some m =>
  composite_has_been_directly_observed ELMO_component som.1 m
    \/
...
Theorem ELMO_components_fixed_equivocation_validating (equivocating_nodes : Ci) :
  forall i : index, i ∉ equivocating_nodes ->
    component_projection_validator_prop ELMO_component
...
Proof.
  ...
  apply (input_valid_transition_destination
    (pre_loaded_with_all_messages_vlsm (ELMO_component i))) in Hvti as Hsi'.
...
Lemma ELMO_fixed_equivocation_subsumption (equivocating_nodes : Ci) :
  weak_input_valid_constraint_subsumption (free_composite_vlsm ELMO_component)
    (ELMO_fixed_equivocation_global_constraint equivocating_nodes)
    (fixed_equivocation_constraint ELMO_component equivocating_nodes).
Admitted.
...
Lemma ELMO_fixed_equivocation_VLSM_eq (equivocating_nodes : Ci) :
  VLSM_eq
    (composite_vlsm ELMO_component (ELMO_fixed_equivocation_global_constraint equivocating_nodes))
    (composite_vlsm ELMO_component (fixed_equivocation_constraint ELMO_component equivocating_nodes)).
Proof.
  unfold composite_vlsm.
  split; apply (weak_constraint_subsumption_incl (free_composite_vlsm ELMO_component)).
...
Corollary ELMO_fixed_byzantine_traces_equivocation_char :
  ...
  forall (bis : composite_state ELMO_component) (btr : list (composite_transition_item ELMO_component)),
  fixed_byzantine_trace_alt_prop ELMO_component byzantine_nodes (ELMO_A idx) Message_sender bis btr
    ->
  exists eis etr,
    finite_valid_trace (composite_vlsm ELMO_component (ELMO_fixed_equivocation_global_constraint byzantine_nodes)) eis etr
    ∧ SubProjectionTraces.composite_state_sub_projection ELMO_component
        (elements (list_to_set (finite.enum index) ∖ byzantine_nodes)) eis =
      SubProjectionTraces.composite_state_sub_projection ELMO_component
        (elements (list_to_set (finite.enum index) ∖ byzantine_nodes)) bis
      ∧ SubProjectionTraces.finite_trace_sub_projection ELMO_component
...
Proof.
  ...
  - intros i Hi.
    apply component_projection_validator_is_message_validator,
      ...
      [| by apply ELMO_components_fixed_equivocation_validating].
    by apply ELMO_fixed_equivocation_subsumption.
  - revert Hetr.
