...
Proof.
  ...
  ; split; [done | | done |]
  ; destruct (transition _ _ _) as (si', om'); inversion_clear Ht.
  - exists (Existing i).
    simpl.
...
Proof.
  ...
  - destruct l as [nsi | ieqvi li | ieqvi li]
    ...
    + simpl.
      ...
      split; [done |].
      destruct (transition _ _ _) as (si', _output).
      inversion Ht. subst.
...
Proof.
  ...
  - simpl in Hprefix.
    ...
    simpl in IHbprefix |- *; rewrite IHbprefix.
...
Lemma equivocator_valid_transition_project_inv2
  ...
  : exists (i : nat), di = Existing i /\
    ...
    valid X lx (s'x, iom) /\ transition X lx (s'x, iom) = (sx, oom).
Proof.
  ...
  ; (repeat split; [done.. |])
  ; destruct (transition X _ _) as (s'j', _oom) eqn: Hti.
...
Lemma equivocator_valid_transition_project_inv4
  ...
  : exists si, equivocator_state_project s i' = Some si /\
    exists (oitem : option (transition_item X)),
...
Lemma equivocator_valid_transition_project_inv5
  ...
  : exists (i : nat) si, equivocator_state_project s i = Some si /\
    exists (itemx : transition_item X),
...
Proof.
  ...
  - specialize (existing_false_label_equivocator_transition_size X Ht _ Heqs'i) as Ht_size.
    ...
    simpl in Ht_pr.
    destruct (transition X _ _) as (si', _oom) eqn: Hti.
...
Proof.
  ...
  - exists [].
    eexists; split; [done |].
    eexists; split; [done |].
    apply (finite_valid_trace_from_to_empty (preloaded_vlsm X seed)); revert Hj.
  ...
  - remember {| l := l; input := iom; |} as item.
    ...
    + specialize (Hchar2 _ eq_refl) as [HvX HtX].
      ...
      apply (finite_valid_trace_from_to_extend (preloaded_vlsm X seed)); [done |].
...
Proof.
  apply (VLSM_incl_finite_valid_trace_from_to
    (proj1 (vlsm_is_preloaded_with_False (equivocator_vlsm X)))) in Hbtr.
  ...
  - destruct Hdi as [Hsn Htr].
    ...
    apply (VLSM_incl_finite_valid_trace_from_to (proj2
      (vlsm_is_preloaded_with_False X))) in Htr.
    by clear -Htr; destruct X.
  - destruct Hdi as [s [Hpr_bs_i Htr]].
    eexists; split; [done |].
    apply (VLSM_incl_finite_valid_trace_from_to (proj2 (vlsm_is_preloaded_with_False X)))
      in Htr.
    by clear -Htr; destruct X.
Qed.

(**
  The projection of a segment of a valid trace from the [preloaded_with_all_messages_vlsm]
  corresponding to the [equivocator_vlsm] is defined and it is a valid
  trace segment in the [preloaded_with_all_messages_vlsm] corresponding to the original vlsm.
*)
...
Lemma preloaded_equivocator_vlsm_valid_trace_project_inv2
  ...
  : exists fsj, equivocator_state_project fs j = Some fsj /\
    ...
    | Existing i =>
      ...
      (initial_state_prop (preloaded_with_all_messages_vlsm (equivocator_vlsm X)) is ->
...
Proof.
  ...
  - unfold equivocator_label_zero_project in H.
    ...
    + rewrite equivocator_state_project_zero in H0.
      by destruct (transition _ _ _); inversion_clear H0.
    + by destruct (equivocator_state_project _ _); [destruct (transition _ _ _) |];
        inversion_clear H0.
  - by apply H.
...
Proof.
  ...
  - unfold equivocator_label_zero_project in H.
    ...
    + by destruct (equivocator_state_project _ _); [destruct (transition _ _ _) |];
        inversion_clear H0.
    + by rewrite equivocator_state_project_zero in H0; destruct (transition _ _ _);
        inversion_clear H0.
    + by destruct (equivocator_state_project _ _); [destruct (transition _ _ _) |];
        inversion_clear H0.
  - apply H.
