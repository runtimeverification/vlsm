...
(** * Core: Fixed Set Equivocation
  ...
  Assuming that a only a fixed subset of the components (here called _equivocating_)
  ...
  to free composition, we can say that a message can be produced by components allowed
  to equivocate, if it can be produced by their free composition
...
(** ** Restricting Fixed valid traces to only the equivocators
  ...
  when projected to the composition of equivocator components.
...
(** ** Changing the behavior of equivocators within a trace
  ...
  only depends on the messages sent by non-equivocating components, it makes it much
...
(** ** Fixed equivocation over an empty set
  ...
  If the set of components allowed to equivocate is empty, intuitively, it follows
  that no message equivocation is allowed, since that would mean that at least
  one component would equivocate.
*)
